...
CFG
  ...
  if [[ -f "${SRC}/${NAME}.service" ]]; then
    ...
    sed -E \
      -e "s#/etc/${NAME}/${NAME}.conf#/etc/${NAME}/${NAME}.conf#g" \
      -e "s#/usr/bin/${NAME}#${BIN}/${NAME}#g" \
      "${SRC}/${NAME}.service" | with_sudo tee "${unit}" >/dev/null
  else
    # fallback minimal unit
    with_sudo tee "${unit}" >/dev/null <<UNIT
[Unit]
Description=${NAME} rogue AP orchestrator
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
EnvironmentFile=-/etc/${NAME}/${NAME}.conf
ExecStart=${BIN}/${NAME} \${ARGS}
Restart=on-failure
...
post() {
  log "[${NAME}] post: smoke"
  "${BIN}/${NAME}" --help >/dev/null || warn "[${NAME}] --help returned non-zero"
  command -v hostapd-mana >/dev/null 2>&1 || warn "[${NAME}] hostapd-mana still missing (required by upstream)"
  log "[${NAME}] tip: set ARGS in /etc/${NAME}/${NAME}.conf then: sudo systemctl enable --now ${NAME}"
...
uninstall() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    with_sudo systemctl disable --now "${NAME}" 2>/dev/null || true
    rm_if_exists "${SYSTEMD}/${NAME}.service"
    with_sudo systemctl daemon-reload 2>/dev/null || true
  fi

  log "[${NAME}] killing orchestrator + helpers"
  with_sudo pkill -x "${NAME}" 2>/dev/null || true
  ...
  with_sudo rm -rf /etc/${NAME} 2>/dev/null || true
